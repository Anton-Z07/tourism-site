var topMenu = {
    structure: {
        ...
        "Утилиты": {
            ...
            "Обновление счётчиков статей": {_action:"countarticles"}
        },
        "Тесты": {
    ...
    select: function(action, model) {
        ...
        switch(action) {
            ...
            case 'bindentities':
                ...
                '<div style="margin-bottom: 5px;">После добавления или редактирования статей необходимо выполнить их привязку.<br/>Скрипт создаёт только автоматические связи. Время работы - до 10 секунд.<br/></div>'+
            ...
            case 'countarticles':
                ...
                $('<div style="width:100%; height: 50px; margin: 20px;">'+
                '<div style="margin-bottom: 5px;">После добавления или изменения статуса публикации статей необходимо выполнить обновление счётчиков.<br/>Скрипт учитывает только опубликованные статьи. Время работы - до 5 секунд.<br/></div>'+
                '<form method="post" class="form-countarticles">'+
                        '<input type="submit" value="Обновить счётчики"/>'+
                    '</form>'+
                '</div>'+
                '<div class="ttsd loadingcontainer"></div>').appendTo(content);
                ...
                content.find('.form-countarticles').bind('submit', function() {
                    if(!container.hasClass('loading')) {
                        container.addClass('loading');
                    }

                    $.ajax('/admin/countarticles/', {
                        type: 'GET',
                        cache: true,
                        dataType: "json",
                        contentType: "application/json",
