...
class PackageProcessor {
    ...
    const CITY_AREA_TYPE     = 2;
    const REGION_AREA_TYPE   = 1;
    const COUNTRY_AREA_TYPE  = 0;
    ...
    private function setAreaType($type)
    {
        $this->area_type = $type;
    }

    private function setLandmarkAreaStack($stack)
    {
        $this->landmark_area_stack   = $stack;
        $this->landmark_area_stack[] = $this->area_id;
    }

    private function getLandmarkIm($im)
    {
        $landmark_im = $im;

        if ($this->area_type == self::CITY_AREA_TYPE)
        {
            $landmark_im = $this->area_id;
        }
        else if ($this->area_type == self::REGION_AREA_TYPE)
        {
            $landmark_im = implode(',', $this->landmark_area_stack);
        }
        else if ($this->area_type == self::COUNTRY_AREA_TYPE)
        {
            // ничего не меняем
        }
    ...
    protected function buildPackage($package)
    {
        ...
        if (!$this->core)
        {
            $this->setAreaId($package->area_id);
            $this->setAreaType($package->area->area_type);
        }
    ...
    protected function runAreaStack()
    {
        ...
        if (!$this->core)
        {
            // отдельный стэк для сбора достопримечательностей
            $this->setLandmarkAreaStack($lowerStack);
        }

        $this->areaStack = [];  
        
        if(count($upperStack) > 0) {
            // берём все по связям сверху
            if(!$this->core) {
    ...
    protected function runAreaFGStack()
    {
        ...
        if (!$this->core)
        {
            // не грузим лишние достопримечательности
            $lm_im = $this->getLandmarkIm($im);
        }
        else
        {
            $lm_im = $im;
        ...
        foreach($this->connection->select('select * from landmark_area as la where la.area_id in ('.$lm_im.')') as $item) {
